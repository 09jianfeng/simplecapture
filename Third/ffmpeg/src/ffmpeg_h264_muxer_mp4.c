...
static void h264MuxerOpenVideo(AVFormatContext *oc, AVCodec *codec, H264MuxerOutputStream *ost, AVDictionary *opt_arg, const void * spsData, const int spsLen, const void * ppsData, const int ppsLen) {
    ...
    memcpy(&sps_pps[4], spsData, spsLen);
    h264MuxerSetStartCode(&sps_pps[4 + spsLen]);
    memcpy(&sps_pps[8 + spsLen], ppsData, ppsLen);
    c->extradata_size = 8 + spsLen + ppsLen;
    c->extradata = (uint8_t*)av_malloc(8 + spsLen + ppsLen + AV_INPUT_BUFFER_PADDING_SIZE);
    memcpy(c->extradata, sps_pps, 8 + spsLen + ppsLen);
    av_dict_free(&opt);
    if (ret < 0) {
...
H264MuxerHandler * h264MuxerInitParams(const int bitrate, const int width, const int height, const int frameRate) {
...
int h264MuxerInitOutputPath(H264MuxerHandler * h264MuxerHandler, const char * outputFilePath, const int outputFilePathLen) {
    if (h264MuxerHandler == NULL) {
...
void h264MuxerWriteVideo(H264MuxerHandler * h264MuxerHandler, const void * h264Data, const int h264DataLen, const int isKeyFrame, const void * spsData, const int spsLen, const void * ppsData, const int ppsLen, const float pts) {
    if (h264MuxerHandler == NULL) {
        fprintf(stderr, "Error h264MuxerHandler is null\n");
        return;
    }
    ...
    pkt.pts = pts / av_q2d(videoSt->st->time_base);
    pkt.pos = -1;
