...
{
    av_log(NULL, AV_LOG_ERROR, "crash in ffmpeg");
    if (program_exit)
        program_exit(ret);
    if (ret != 0) {
        longjmp(jmpbuffer, ret);
    }
    //mark by bhl
    //    exit(ret);
}
...
//add bhl
int ffmpeg_show_help(void *optctx, const char *opt, const char *arg)
{
    char *topic, *par;
    av_log_set_callback(log_callback_help);
    ...
    if (!*topic) {
        ffmpeg_show_help_default(topic, par);
    } else if (!strcmp(topic, "decoder")) {
        show_help_codec(par, 0);
    } else if (!strcmp(topic, "encoder")) {
        show_help_codec(par, 1);
