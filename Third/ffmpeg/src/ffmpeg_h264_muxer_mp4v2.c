...
static void h264MuxerAddStream(H264MuxerHandler * h264MuxerHandler, H264MuxerOutputStream *ost, AVFormatContext *oc, AVCodec **codec, enum AVCodecID codec_id) {
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    switch ((*codec)->type) {
            ...
            if (h264MuxerHandler->audioSampleRate > 0) {
                c->sample_rate = h264MuxerHandler->audioSampleRate;
            }else{
                c->sample_rate = 44100;
            }
            ...
            if (h264MuxerHandler->audioChannels > 0) {
                c->channels       = h264MuxerHandler->audioChannels;
            }else{
                c->channels = 1;
            }
...
static void h264MuxerOpenVideo(AVFormatContext *oc, AVCodec *codec, H264MuxerOutputStream *ost, AVDictionary *opt_arg, const void * spsData, const int spsLen, const void * ppsData, const int ppsLen) {
    ...
    size_t sps_pps_len0 = sizeof(unsigned char) * (8 + spsLen + ppsLen);
    unsigned char * sps_pps = (unsigned char *)malloc(sps_pps_len0);
    memset(sps_pps, 0, sps_pps_len0);
...
static int h264MuxerCreateMp4(H264MuxerHandler * h264MuxerHandler, const void * spsData, const int spsLen, const void * ppsData, const int ppsLen) {
    ...
    if (h264MuxerHandler->outputFilePath) {
        free(h264MuxerHandler->outputFilePath);
        h264MuxerHandler->outputFilePath = NULL;
    }
    
    if (h264MuxerHandler->meta) {
        free(h264MuxerHandler->meta);
        h264MuxerHandler->meta = NULL;
    }
...
H264MuxerHandler * h264MuxerInitOutputPathV2(const char * outputFilePath, const int outputFilePathLen,const char *meta ,const int metaLen) {
	...
	H264MuxerHandler * h264MuxerHandler = malloc(sizeof(H264MuxerHandler));
    memset(h264MuxerHandler, 0, sizeof(H264MuxerHandler));
	 if (h264MuxerHandler == NULL) {
		 return NULL;
	 }
    ...
    h264MuxerHandler->meta  = malloc(sizeof(char*) * 1024);
    memset(h264MuxerHandler->meta, 0, sizeof(char) * 1024 * 3);
    memcpy(h264MuxerHandler->meta, meta, metaLen);
...
void h264MuxerWriteVideoV2(H264MuxerHandler * h264MuxerHandler, const void * h264Data, const int h264DataLen,
...
void h264MuxerWriteAudioV2(H264MuxerHandler * h264MuxerHandler, const void * aacData, const int aacDataLen) {
    ...
    if (h264MuxerHandler == NULL) {
        fprintf(stderr, "264 muxer write audio error h264MuxerHandler is null\n");
        return;
    }
    
    if (aacData == NULL || aacDataLen == 0) {
        fprintf(stderr, "264 muxer write audio error aac no data\n");
        return;
    }
    
    if (h264MuxerHandler->formatContext == NULL) {
        fprintf(stderr, "264 muxer write audio null context\n");
        return;
    }
    ...
    ret = av_interleaved_write_frame(h264MuxerHandler->formatContext, &pkt);
    if (ret < 0) {
        fprintf(stderr, "264 muxer write audio frame failed\n");
    }
}
...
void h264MuxerWriteMetaV2(H264MuxerHandler * h264MuxerHandler, const char*metaKey, const char*metaValue) {
    ...
    av_dict_set(&metaData, "my_meta_key", "my_meta_value", 0);
    h264MuxerHandler->formatContext->metadata = metaData;
    avformat_write_header(h264MuxerHandler->formatContext, &h264MuxerHandler->formatContext->metadata);
}
...
void h264MuxerCloseMp4V2(H264MuxerHandler * h264MuxerHandler) {
    ...
    if ((h264MuxerHandler ->outputFormat) && (!(h264MuxerHandler->outputFormat->flags & AVFMT_NOFILE))) {
