...
//add wqm
static int xprintf_buf_size  = 256*1024;
static int xprintf_buf_pos = 0;
static char* xprintf_buf = NULL;
...
#define xprintf(...) ((void)__android_log_print(ANDROID_LOG_INFO, "mediarecord", __VA_ARGS__))
#else
static void Xprintf(const char* fmt, ...)
{
    // realloc buffer
    if (xprintf_buf_pos + 1024 > xprintf_buf_size) {
        xprintf_buf_size += xprintf_buf_size;
        char * tmp = malloc(xprintf_buf_size);
        memset(tmp, 0, xprintf_buf_size);
        strcpy(tmp, xprintf_buf);
        free(xprintf_buf);
        xprintf_buf = tmp;
    }
    ...
    va_list args;
    va_start(args, fmt);
    vsprintf(xprintf_buf + xprintf_buf_pos, fmt, args);
    va_end(args);
    
    // update pos
    xprintf_buf_pos = strlen(xprintf_buf);
}
#define xprintf(...) \
do {			\
Xprintf(__VA_ARGS__);	\
...
static void default_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    if (!def->nokey)
        xprintf("%s%s=", wctx->section_pbuf[wctx->level].str, key);
    xprintf("%s\n", value);
}
...
static void compact_print_section_header(WriterContext *wctx)
{
    ...
    if (!(section->flags & SECTION_FLAG_IS_ARRAY) && parent_section &&
        ...
        if (compact->print_section &&
            ...
            xprintf("%s%c", section->name, compact->item_sep);
    }
}
...
static void compact_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    if (wctx->nb_item[wctx->level]) xprintf("%c", compact->item_sep);
    if (!compact->nokey)
        xprintf("%s%s=", wctx->section_pbuf[wctx->level].str, key);
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    xprintf("%s", compact->escape_str(&buf, value, compact->item_sep, wctx));
...
static void flat_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    xprintf("%s", wctx->section_pbuf[wctx->level].str);
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    xprintf("%s=", flat_escape_key_str(&buf, key, flat->sep));
    av_bprint_clear(&buf);
...
static void ini_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    xprintf("%s=", ini_escape_str(&buf, key));
    av_bprint_clear(&buf);
...
#define JSON_INDENT() xprintf("%*c", json->indent_level * 4, ' ')
...
static void json_print_section_header(WriterContext *wctx)
{
    ...
    if (section->flags & SECTION_FLAG_IS_WRAPPER) {
        ...
        if (section->flags & SECTION_FLAG_IS_ARRAY) {
            ...
            xprintf("\"%s\": {%s", buf.str, json->item_start_end);
        } else {
            xprintf("{%s", json->item_start_end);

            /* this is required so the parser can distinguish between packets and frames */
            if (parent_section && parent_section->id == SECTION_ID_PACKETS_AND_FRAMES) {
                if (!json->compact)
                    JSON_INDENT();
                xprintf("\"type\": \"%s\"%s", section->name, json->item_sep);
            }
        }
        av_bprint_finalize(&buf, NULL);
    }
                                       ...
                                       const char *key, const char *value)
{
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    xprintf("\"%s\":", json_escape_str(&buf, key,   wctx));
    av_bprint_clear(&buf);
    xprintf(" \"%s\"", json_escape_str(&buf, value, wctx));
    av_bprint_finalize(&buf, NULL);
}
...
static void json_print_int(WriterContext *wctx, const char *key, long long int value)
{
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    xprintf("\"%s\": %lld", json_escape_str(&buf, key, wctx), value);
    av_bprint_finalize(&buf, NULL);
}
...
#define XML_INDENT() xprintf("%*c", xml->indent_level * 4, ' ')
...
static void xml_print_section_header(WriterContext *wctx)
{
    ...
    if (wctx->level == 0) {
        ...
        xprintf("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        xprintf("<%sffprobe%s>\n",
    ...
    if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS) {
        ...
        if (section->flags & SECTION_FLAG_IS_ARRAY) {
            XML_INDENT(); xprintf("<%s>\n", section->name);
        } else {
            XML_INDENT(); xprintf("<%s ", section->name);
            xml->within_tag = 1;
        }
    }
}
...
static void xml_print_str(WriterContext *wctx, const char *key, const char *value)
{
    ...
    if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS) {
        ...
        if (wctx->nb_item[wctx->level])
            xprintf(" ");
        xprintf("%s=\"%s\"", key, xml_escape_str(&buf, value, wctx));
    }
...
static int open_input_file(InputFile *ifile, const char *filename)
{
    ...
    if (!fmt_ctx) {
        ...
        av_log(NULL, AV_LOG_ERROR, "crash in ffprobe");
        exit_program(1);
    }
...
static void print_section(SectionID id, int level)
{
    ...
    xprintf("%*c  %s", level * 4, ' ', section->name);
    if (section->unique_name)
        xprintf("/%s", section->unique_name);
    xprintf("\n");
...
extern int ffmpeg_running;//bhl

char* ffprobe_main(int argc, char **argv)
{
    //bhl
    if (ffmpeg_running) {
        LOGW("ffporbe error :ffmpeg is running");
        return NULL;
    }
    ffmpeg_running = 1;
    //bhl end
    ...
    xprintf_buf = malloc(xprintf_buf_size);
    if (xprintf_buf == NULL)
        return NULL;
    memset(xprintf_buf, 0, xprintf_buf_size);
    xprintf_buf_pos = 0;
...
//    show_banner(argc, argv, options);
    ...
    if (ret < 0) {
        free(xprintf_buf);
        xprintf_buf = NULL;
        LOGI("probe error, return\n");
        ffmpeg_running = 0;
        return NULL;
    } else {
        if (strlen(xprintf_buf) < 1024) {
            LOGI("buf_size:%d, content:\n %s", strlen(xprintf_buf), xprintf_buf);
        } else {
            int len = strlen(xprintf_buf);
            i = 0;
            LOGI("buf_size:%d, content:\n", len);
            while(i+1000<len) {
                char tmp = xprintf_buf[i+1000];
                xprintf_buf[i+1000] = 0;
                LOGD("%s", xprintf_buf+i);
                xprintf_buf[i+1000] = tmp;
                i += 1000;
            };
            //LOGD("%s", xprintf_buf+i);
        }
