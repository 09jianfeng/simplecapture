...
//add bhl
const char ffmpeg_program_name[] = "ffmpeg";
const int ffmpeg_program_birth_year = 2000;
static FFmpegCtx *curFFmpegCtx = NULL;
extern void addTinyVideoGpuFilter(AVFrame *frame,void *,double pts);
int ffmpeg_running = 0;
int ffmpeg_process_cacelled = 0;
//add bhl end
...
//add by bhl
static void ffmpeg_reset(int ret)
{
    nb_filtergraphs = 0;
    nb_output_files = 0;
    nb_output_streams = 0;
  ...
  for (i = 0; i < nb_frames; i++) {
...
#endif
    {
        ...
        while (1) {
            ...
            frame_size = pkt.size;
            //add bbhl
            out_pts = av_rescale_q_rnd(pkt.pts, ost->st->time_base, (AVRational){1,1000}, AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX);
            //add bhl end
            output_packet(of, &pkt, ost);
    ...
    ost->frame_number++;
      ...
      if (curFFmpegCtx) {
          if (curFFmpegCtx->cmd_type == libffmpeg_cmd_webp) {
              event.u.webp_progress_t.frame_num = frame_number;
              event.u.webp_progress_t.frame_pts = out_pts;
              event.type = libffmpeg_event_webp_progress;
          }
          else if (curFFmpegCtx->cmd_type == libffmpeg_cmd_snapshot_multiple) {
              LOGD("frame_num:%d, frame_pts: %lld\n",ost->frame_number, out_pts);
              event.u.snapshot_progress_t.frame_num = frame_number;
              event.u.snapshot_progress_t.frame_pts = out_pts;
              event.type = libffmpeg_event_snapshot_multiple_progress;
          }
          else if (curFFmpegCtx->cmd_type == libffmpeg_cmd_video_concat) {
              event.type = libffmpeg_event_video_concat_progress;
              event.u.concat_progress_t.frame_num = frame_number;
              
          }
          else if (curFFmpegCtx->cmd_type == libffmpeg_cmd_filter) {
              event.type = libffmpeg_event_filter_progress;
              event.u.filter_progress_t.frame_num = frame_number;
          }
          else if( curFFmpegCtx->cmd_type == libffmpeg_cmd_transcode) {
              event.type = libffmpeg_event_transcode_progress;
              event.u.transcode_progress_t.frame_num = frame_number;
          }
          else if ( curFFmpegCtx->cmd_type == libffmpeg_cmd_image_video) {
              event.type = libffmpeg_event_image_video_progress;
              event.u.image_video_progress_t.frame_num = frame_number;
          }
          else if (curFFmpegCtx->cmd_type == libffmpeg_cmd_snapshot_single ) {
              event.type = libffmpeg_event_snapshot_single_progress;
              event.u.snapshot_progress_t.frame_num= frame_number;
          }
          else if(curFFmpegCtx->cmd_type == libffmpeg_cmd_image_filter){
              event.type = libffmpeg_event_image_filter_progress;
              event.u.concat_progress_t.frame_num = frame_number;
          }
          else if (curFFmpegCtx->cmd_type == libffmpeg_cmd_split_screen_merge) {
              event.type = libffmpeg_event_filter_progress;
              event.u.filter_progress_t.frame_num = frame_number;
          }
          else {
              event.type = libffmpeg_event_error;
              break;
          }
          event.user_data = curFFmpegCtx->user_data;
          if (curFFmpegCtx ->ffmpeg_event_cb) {
              curFFmpegCtx->ffmpeg_event_cb(&event);
          }
      }
      //add bhl end
        ...
        while (1) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (debug_ts) {
                    ...
                    //bhl
                    double pts = av_q2d(enc->time_base) * filtered_frame->pts;
//                    av_log(NULL, AV_LOG_INFO, "kelvin test filtered pts=%f",pts);
                    if (curFFmpegCtx && curFFmpegCtx->gpuFilterUser && (filtered_frame->format == AV_PIX_FMT_YUV420P //video
                                                                        || filtered_frame->format == AV_PIX_FMT_YUVJ420P)) { //jpeg
                        addTinyVideoGpuFilter(filtered_frame, curFFmpegCtx->gpuFilterUser, pts);
                    }
                    //bhl end
...
static double rational2ddouble(AVRational r) {
                        ...
                        int *decode_failed)
{
    ...
    //bhl
    //ensure there is valid output, then send the decoded_frame to GPU filters
//    double pts = (double)ist->pts / (double)AV_TIME_BASE;
//    if (curFFmpegCtx && curFFmpegCtx->gpuFilterUser && (decoded_frame->format == AV_PIX_FMT_YUV420P //video
//                                                        || decoded_frame->format == AV_PIX_FMT_YUVJ420P)) { //jpeg
//        addTinyVideoGpuFilter(decoded_frame, curFFmpegCtx->gpuFilterUser, pts);
//    }
    //bhl end
...
static int transcode(void)
{
    ...
    while (!received_sigterm) {
        ...
        // add by hwz
        if (curFFmpegCtx && ffmpeg_process_cacelled) {
            LOGW("cancel, cmd:%d", curFFmpegCtx->cmd_type);
            break;
        }
        // add end
...
int ffmpeg_main(libffmpeg_instance_t *p_md, int argc, char **argv)
{
    int i, ret;
    int64_t ti;
    ...
    //bhl
    if (ffmpeg_running) {
        LOGW("ffmpeg error: ffmpeg is ruuning");
        return -1;
    }
    ffmpeg_running = 1;
    //程序退出时候的跳转代码
    int jret = setjmp(jmpbuffer);
    if (jret !=0 ) {
        //
        LOGE("program exit unexcepted");
        main_return_code = -1;
        goto fail;
    }
    //bhl end
    ...
    if (!p_md) {
        LOGE("ffmpeg_main error");
        main_return_code = -1;
        goto fail;
    }
    curFFmpegCtx = p_md->excontext;
    //add end
...
//    show_banner(argc, argv, options);
    ...
    if (ret < 0){
        exit_program(1);
        av_log(NULL,AV_LOG_ERROR,"ffmpeg_parse_options error");//bhl
        main_return_code = -1;
        goto fail;
    }
    ...
    if (nb_output_files <= 0 && nb_input_files == 0) {
        ...
        exit_program(1);
        av_log(NULL,AV_LOG_ERROR,"nb_output_files is 0 ");
        main_return_code = -1;
        goto fail;
    }
...
fail:
    curFFmpegCtx = NULL;
    // reset
    int return_code = main_return_code;
    LOGD("main_return_code:%d",  main_return_code);
    ffmpeg_reset(0);
    ffmpeg_running = 0;
...
//add bhl
int ffmpeg_av_get_frame_filename(char *buf, int buf_size, const char *path, int number)
{
    const char *p;
    char *q, buf1[20], c;
...
libffmpeg_instance_t *ffmpeg_new(FFmpegCtx *ctx)
{
    libffmpeg_instance_t *p_instance = malloc(sizeof(libffmpeg_instance_t));
    if (ctx) {
        p_instance->excontext = ctx;
    }
    return p_instance;
}
void ffmpeg_release(libffmpeg_instance_t *p_instance)
{
    if (p_instance) {
        free(p_instance);
    }
