...
- (void)fetchVideoPathFromAsset:(id)asset result:(void (^)(NSString *path, NSError *error))result progressblock:(void (^)(float progress))progressblock{
    ...
    [self getVideoPathFromPHAsset:phasset fileName:fileName fileDir:@"originvideo" rootDir:rootDir complete:^(NSString *path, NSString *filename){
        result(path,nil);
...
-(void)getVideoPathFromPHAsset:(PHAsset *)asset fileName:(NSString *)fileName fileDir:(NSString *)fileDir rootDir:(NSString *)rootDir complete:(void (^)(NSString *, NSString *))result failure:(void (^)(NSString *failInfo, NSError *error))failure cancell:(void (^)(void))cancell progressblock:(void (^)(float progress))progressblock{
    ...
    [manager requestExportSessionForVideo:asset options:options exportPreset:AVAssetExportPresetMediumQuality resultHandler:^(AVAssetExportSession * _Nullable exportSession, NSDictionary * _Nullable info) {
        NSString *savePath =  [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"target_%@",fileName]];
