...
@interface VideoFileKit : NSObject
...
/**
 获取指定目录下所有含有指定的扩展名的文件名
 
 @param directoryPath 目录路径
 @param pathExtension 扩展名
 @return 含有指定的扩展名的文件名数组，获取失败或者目录下不存在符合要求的文件返回nil
 */
...
/**
 ...
 @param directoryPath 目录路径
 @param pathExtension 扩展名数组
 @return 含有指定的扩展名数组内任一扩展名的文件名数组，获取失败或者目录下不存在符合要求的文件返回nil
 */
...
/*
 * @brief 输入一个文件路径,返回文件URL(目前主要用于asset url)
 * @param path 文件路径(由于目前传入的path,有的带file,有的不带,初始化不对,会导致asset初始化失败)
 */
