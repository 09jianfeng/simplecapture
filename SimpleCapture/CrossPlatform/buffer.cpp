...
static MediaMutex   gCacheLock("BufferMutex");
static stdCacheMap  gCacheInfos;
static bool gEnableBuffCache = false;

void MediaLibrary::setEnableBuffCache(bool isEnable)
{
	gEnableBuffCache = isEnable;
}
...
void MediaLibrary::FreeBuffer(void *buffer)
{
	...
	if (gEnableBuffCache)
	{
		...
		{
			stdPointerList& iBufList = iter->second->iBufList;
			if (std::find(iBufList.begin(), iBufList.end(), header) == iBufList.end())
			{
				iBufList.push_back(header);
			}
			buffer = NULL;
		}
		gCacheLock.Unlock();
